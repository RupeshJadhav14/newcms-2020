...
//unset session if specify
    ...
    public static function isUserLogin() {
        if (isset($_SESSION[CFG::$session_key]['username']) && trim($_SESSION[CFG::$session_key]['username']) != "")
            return true;
        else
            return false;
    }
     ...
     */
    public static function FrontdoPaging($recordCountVar, $fields, $tables, $where = "", $whereParam = array(), $order = "", $group_by = "") {
...
// store current state in session
                ...
                if (isset($_GET['getCount']) && $_GET['getCount'] == 1) {
					if (trim($group_by) != '') {
                        echo json_encode(array("result" => "success", "records" => $sqlData, "totalRecords" => UTIL::dataCountGroupBy($tables, $where, $whereParam, $group_by)));
                    } else {
				...
				if (trim($group_by) != '') {
                    Layout::addFooter('<script type="text/javascript">var ' . $recordCountVar . ' = ' . UTIL::dataCountGroupBy($tables, $where, $whereParam, $group_by) . '</script>');
                } else {
     ...
     * @author Rutwik Avasthi <php.datatechmedia@gmail.com>
     */
    public static function doPaging($recordCountVar, $fields, $tables, $where = "", $whereParam = array(), $order = "", $group_by = "") {
...
// store current state in session
	...
	/*
     * added by Keyur Mistry
     * Purpose: Total record count used with group by clause.
     */

    public static function dataCountGroupBy($tables, $where, $whereParam, $group_by = "") {
        $sql = "select count(" . $group_by . ") as recordCount from " . $tables;
...
';
    ...
    public static function loadTinymce($image = '0', $id) {?>
        ...
        <div class="descBtns">	
            <?php if ($image == '1') { ?>                                                     
                <input type="button" id="html-editor" onclick="tinymce.execCommand('mceImage', false, 'txaDescription');" value="Insert Image" title="Insert Image" class="btn backBtn"/> 
            <?php } ?>
            </div>     
            ...
            <script type="text/javascript">
                $(document).ready(function() {
                    ...
                    plugins: ["advlist autolink lists link image charmap print preview anchor hr paste","searchreplace visualblocks code fullscreen code","insertdatetime media table contextmenu paste textcolor template imagetools"],
                    toolbar: " insertfile undo redo | template | hr | styleselect | bold italic underline | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link media | forecolor backcolor | mybutton | image | code",
                    forced_root_block: "",
                    link_class_list: [{title: 'None', value: ''},{title: 'cmsButton', value: 'cmsButton'}],
                    templates: [{title: '2 Columns', description: 'Adds a 2 columns table', url: "<?php echo $path; ?>/plugins/tiny_mce/themes/advanced/template/two_column.html"},{title: '3 Columns', description: 'Adds a 3 columns table', url: "<?php echo $path; ?>/plugins/tiny_mce/themes/advanced/template/three_column.html"}],
...
// check action table for both front and admin to find that action is home page or not
	...
	function ResizeThisImage($src,$dest,$w,$h)
	{
		/*
			$path = URI::getAbsTemplatePath().'/libraries/image/resize_image.php';
			include_once($path);
			...
			*/
			...
			imagejpeg($tci, $dest, 80);
				
	}
	
	
	function CompressThisImage($source_url, $destination_url, $quality) 
	{

		//$url = 'media11/'.$_FILES["file"]["name"];
		...
		$info = getimagesize($source_url);
   		...
   		elseif ($info['mime'] == 'image/png')
			...
			if ($image2 !== FALSE) {
				imagepng($image2, 'example-cropped.png');
				imagedestroy($image2);
			}
			imagedestroy($image);
...
//echo "<pre>";print_r($arrFile);exit;
    ...
    public static function displayUploadNote($showSize) {
        if (isset(CFG::$imageSize[$showSize])) {
            list($width, $height) = CFG::$imageSize[$showSize];
            return "Allowed Extensions: jpg, gif, png<br>Recommended Size: Width:" . $width . "px, Height: " . $height . "px";
        }
        return '';
    }
