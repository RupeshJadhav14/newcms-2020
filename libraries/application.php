...
//echo "<pre>";
//print_r($_SESSION[CFG::$session_key]); exit; 
// check user is super administrator or not
                if ($_SESSION[CFG::$session_key]['access_level'] == 'all') {
// do nothing
                } else if ($_SESSION[CFG::$session_key]['access_level'] == APP::$appType && $_SESSION[CFG::$session_key]['access_type'] == 'all') { // check application level access of user. eg. front or admin
// do nothing
                } else if ($_SESSION[CFG::$session_key]['access_type'] == 'selected') {
                    if (in_array(APP::$actionId, $_SESSION[CFG::$session_key]['action']) == false){
                        UTIL::invalidAccess();
                    }
                } else if ($_SESSION[CFG::$session_key]['access_type'] == 'exclude') {
                    if (in_array(APP::$actionId, $_SESSION[CFG::$session_key]['action']) == true){
                        UTIL::invalidAccess();
                    }
                }else{
                    UTIL::invalidAccess();
