...
//    $('a[href|="' + window.location.hash + '"]').tab("show");
    ...
    jQuery.fn.numericOnly =
            function()
            {
                ...
                {
                    if ($(this).hasClass('select_text')) {
                        $(this).focus(function() {
                            $(this).select();
                        });
                    }
                    $(this).on('dragstart drop', function() {
                        return false;
                    })
                    /* ends here */
                    $(this).keydown(function(e)
                    {
                        ...
                        if ($(this).hasClass('hasDatepicker') || $(this).hasClass('isDate')) {
                            return false;
                        }
                        ...
                        if ($(this).hasClass('numbers') && (key == 110 || key == 190)) {
                            return false;
                        }

                        /*Ratan Desai
                         * 05-07-17
                         * Allow two space in phone/mobile number if "phoneNumbers" class applied
                         */
                        if ($(this).hasClass('phoneNumbers')) {
                            if (key == 110 || key == 190) {
                                return false;
                            }
                            else if (key == 32) {
                                if (($(this).val().split(" ").length <= 2 && !/(.*)\s+$/.test($(this).val()))) {
                                    allowSpace = true;
                                }
                            }
                        }
                        ...
                        if ($(this).hasClass('decimalPrice')) {
                            var text = $(this).val();
                            var len = text.length;

                            if ((text.indexOf('.') == -1) && e.which != 110) {
                                if (len > 6) {
                                    e.preventDefault();
                                }
                            } else {
                                if (text.indexOf('.') != -1) {
                                    var decAfter = parseInt(text.indexOf(".")) + 4;
                                    if (len > decAfter || e.which == 110) {
                                        e.preventDefault();
                                    }
                                }
                            }
...
// home, end, period, and numpad decimal                       
	...
	/* Create by Keyur Mistry <keyur.datatech@gmail.com> 
     @Date: 27-11-2016
     */
    jQuery.fn.load_ajax_select = function(obj) {
        var selector = $(this);
        //var is_search = ajaxOptions.search;
        //var module = ajaxOptions.module;
        selector.select2({
            minimumInputLength: 1,
            width: selector.data('width') || "100%",
            ...
            ajax: {
                ...
                data: function(params) {
                    return {
                        name: params.term,
                        //notid: selector.data('selected_ids'),
                        projectid: selector.data('projectid'),
                        notid: notinids,
                        action: selector.data('action')// search term
                    };
                },
            ...
            /*containerCssClass : "bigdropFocus",
            ...
            language: {
                noResults: function() {
                    callback_event = selector.data('callback_event');
                    callback_event_title = selector.data('callback_event_title');
                    returnHtml = '<span>No matches found</span>'
                    if (callback_event != undefined && callback_event != "") {
                        callback_event_title = callback_event_title ? callback_event_title : "Create New";
                        //returnHtml += '<br/><a href="javascript:;" onclick="window[callback_event](this)" title="Create New Product"><strong>Create New Product</strong></a>';
                        returnHtml += '<br/><a href="javascript:;" onclick="window[callback_event](this)" title="' + callback_event_title + '" class="create_new_link"><strong>' + callback_event_title + '</strong></a>';
                        //return 'Result Not Found. <a href="javascript:;" onclick="window[noresult](this)" title="Result Not Found. Create New"><strong>Create New</strong></a>';
                    }
                    return returnHtml;
                }
            },
			...
			if(selector.select2('data') != undefined && selector.select2('data')[0] != undefined) {
				var prev_id = selector.select2('data')[0].id;
				if (prev_id != undefined && prev_id != "")
				    delete notinids[prev_id];
			}
        }).on("select2:select", function(e) {
            var dataObj = e.params.data;
            var ele_id = selector.attr("id");
            if (selector.data('remove_error') == "true" || selector.data('remove_error') == true)
                $("label[for=" + ele_id + "].text-error").remove();
            else
                $("label[for=" + ele_id + "].text-error").hide();

            /* this is to pass already selected values from add more options to ignore previously selected products .*/
            if (selector.data('selected_ids') == true) {
                notinids[dataObj.id] = dataObj.id;
            }
            /* this is to pass already selected values from add more options to ignore previously selected products .*/

            if (selector.data('callback') != "") {
                window[selector.data('callback')](dataObj, selector);
            }
        })

        /* to open dorpdown by default */
        $(selector).next('.select2').find('.select2-selection').one('focus', select2Focus).on('blur', function() {
            $(this).one('focus', select2Focus);
        });
        /* to open dorpdown by default */

        selector.on('select2:selecting', recordFocus);
        selector.on('select2:unselecting', recordFocus);
        selector.on('select2:select', setFocus);
        selector.on('select2:unselect', setFocus);
        selector.on('select2:close', recordFocus);
    }
...
//		var checkedStatus = $(this).find('span').hasClass('checked');
    ...
    if ($(window).width() < 768) {
        ...
        /*$('.custom-table .checkbox').change(function() {
         console.log('hi');
...
function sendAjaxSave(id, fields, tableName, ajaxUrl)
{
    $.ajax({
        type: 'POST',
        url: ajaxUrl,
        data: {id: id, fields: JSON.stringify(fields), table: tableName, action: 'quickSave'},
...
function createUploader(uploaderId, progressBarClass, fileGridId, fileGridFunction, uploadURL, fileTypes, uploadDir, deleteURL, fileData, recordId, maxFiles)
{
    ...
    if (recordId != "")
    {
        ...
        $.each(fileData, function(fileKey, curFile) {
            ...
            if (curFile.name != "")
            {
                ...
                //adde by keyur mistry on 03-03-2017 
                if(maxFiles)
                    totalFiles++;
    ...
    $('#' + uploaderId).fileupload({
        ...
        add: function(e, data) {
            ...
            {
				// disable upload file button when uploading any image.
			...
			// enable upload file button when uploading is done.
			//$("#" + uploaderId).attr("disabled", false);
			disableEnableFileUploadButton(uploaderId, false);
        },
...
function displayUploadResult(file, deleteURL, uploaderId, recordId, propURL)
{
    ...
    var fileImage = '<img class="imgMain" src="' + file.url + '" onclick="displayImgProp(this,\'' + file.name + '\',\'' + uploaderId + '\')"/>';       // display image with property fuction                
...
//    });
    ...
    if ($(window).width() < 768) {
        $('.custom-table .checkbox').off('click');
        $('.custom-table .checkbox').on('click', function() {
            if ($('.mobiAction').is(':hidden')) {
                $('.scrollTop').addClass('Btmspace');
                $(".mobiAction").slideDown().addClass('actionShow');
            }
            $('.counterDiv').html($('.custom-table .checkbox:not(#masterCh)').filter(':checked').length);
        });
        ...
        $('.custom-table li.commentLi').remove();
	...
	/* accodaint */
	$('#div_color1').parent().addClass('showLi');
