        ...
        var displayPages = function(data, startIndex) {
            ...
            for (i = 0; i < data.records.length; i++)
            {
              //  var odd = (i % 2 == 0) ? "" : " class='odd' ";
                ...
                list = list + '<td class="pageName"><a href="index.php?m=mod_comment&a=comment_edit&id=' + data.records[i].id + '"  title="' + data.records[i].customer_name + '">' + data.records[i].customer_name + '</a><ul class="optUl"><li><a href="index.php?m=mod_comment&a=comment_edit&id=' + data.records[i].id + '" title="Edit">Edit</a></li><li><a href="#" onclick="return changeStatus(\'index.php?m=mod_comment&a=change_status\',' + data.records[i].id + ',\'\')" title="Change Status">Change Status</a></li><li class="delLi"><a href="#" onclick="deleteRecord(\'index.php?m=mod_comment&a=delete_comment\',' + data.records[i].id + ')" title="Delete">Delete</a></li></ul><a class="toggleLink" href="javascript:;"></a></td>';
                list = list + '<td class="" data-title="Sort Order:" align="center"><input class="sortTxt text-center inner-page-checkbox" type="text" maxlength="3" id="sort_order'+data.records[i].sort_order+'" name="sort_order['+data.records[i].id+']" value="'+data.records[i].sort_order+'" onblur="if(this.value!='+data.records[i].sort_order+'){changeSortOrder(\'index.php?m=mod_comment&a=save_sortorder\',this.value,'+data.records[i].id+');}if(this.value.length==0){this.value='+data.records[i].sort_order+';}" onkeypress=\'return isNumberic(event);\' /></td>';
...
<?php Core::displayMessage("actionResult", "Comment Save"); ?>
        ...
        function restoreSearchForm()
        {
            ...
            {
                $("#comment_name").val(oldPageState.searchData.searchForm.comment_name);
