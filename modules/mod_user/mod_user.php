...
class User {
    ...
    //this userLogin() is for front-end user
    function userLogin()
        {
            
        Load::loadModel("user");
        //echo "hello Rupesh"; exit();
        // echo "response";
        // print_r($_REQUEST);
         
        //$contactenqObj = new UserModel();
        $userFrontObj = new UserModel();

        $userFrontObj->loginFrontUser($_REQUEST['username'],$_REQUEST['password']);
            
        //$jsData = Layout::bufferContent(URI::getAbsModulePath()."/js/order.php");
        $jsData = Layout::bufferContent(URI::getAbsModulePath()."/js/userLogin.php");
