...
class UserModel {
    ...
    function saveProfile() {
        if (isset($_POST['name'])) {
            ...
            $arrFields = array("name" => $_POST['name'], "username" => $_POST['username'], "email" => $_POST['email'], "phone" => StringManage::removeWhitespace($_POST['phone']), "mobile" => StringManage::removeWhitespace($_POST['mobile']), "fax" => StringManage::removeWhitespace($_POST['fax']), "address" => $_POST['address'], "suburb" => $_POST['suburb'], "state" => $_POST['state'], "country" => (int) $_POST['country'], "postcode" => $_POST['postcode']);

            if (isset($_POST['password']) && trim($_POST['password']) != "")
                $arrFields['password'] = Core::encryptPass($_POST['password']);

            // update record
            DB::update(CFG::$tblPrefix . "user", StringManage::processString($arrFields), " id=%d ", APP::$curId);
    ...
    public function loginFrontUser($username,$password){
        //var_dump($_REQUEST);exit;
        //SELECT * FROM `dtm_user` WHERE username = 'myAdmin' AND password ='6CD5zKeIjvdpJeCoF8FMNA=='
        $sql = "SELECT * FROM " . CFG::$tblPrefix . "user where username ='$username' AND `password` = '" .Core::encryptPass($password) . "' ";
        //var_dump($sql);
        ...
        //var_dump($rowcount);
         if($rowcount >= 1){
                ...
                //UTIL::redirect(URI::getURL(CFG::$livePath)); exit;
    ...
    public function logoutFrontUser(){
        ...
        UTIL::redirect('http://localhost/newcms-2020');exit;
        //UTIL::redirect(URI::getURL("mod_order","order")); exit;
    ...
    public function getUserList() {
        ...
        if (isset($_GET['searchForm']['user_name']) && trim($_GET['searchForm']['user_name']) != "" && isset($_GET['searchForm']['role']) && trim($_GET['searchForm']['role']) != "") {
            ...
            $whereParam["name"] = StringManage::processString($_GET['searchForm']['user_name']);
            $whereParam["username"] = StringManage::processString($_GET['searchForm']['user_name']);
            $whereParam["email"] = StringManage::processString($_GET['searchForm']['user_name']);
            $whereParam["roll"] = StringManage::processString($_GET['searchForm']['role']);
            $flag = false;
    ...
    public function getUserAllData($id) {
        return DB::queryFirstRow("SELECT id,name,username,password,email,suburb,state,country as country_id,address,postcode,phone,mobile,fax,roll_id,active  FROM " . CFG::$tblPrefix . "user where id=%d ", $id);
    }
    ...
    public function saveUser() {
        if (isset($_POST['name'])) {
            ...
            $arrFields = array("name" => $_POST['name'], "email" => $_POST['email'], "username" => $_POST['username'], "phone" => StringManage::removeWhitespace($_POST['phone']), "mobile" => StringManage::removeWhitespace($_POST['mobile']), "fax" => StringManage::removeWhitespace($_POST['fax']), "address" => $_POST['address'], "suburb" => $_POST['suburb'], "state" => $_POST['state'], "country" => (int) $_POST['country'], "postcode" => $_POST['postcode'], "active" => $varStatus, "roll_id" => (int) $_POST['role']);
            ...
            if (APP::$curId == "") {
                ...
                // insert record
                DB::insert(CFG::$tblPrefix . "user", StringManage::processString($arrFields));
...
//                UTIL::redirect(URI::getURL(APP::$moduleName, APP::$actionName, APP::$curId));
    ...
    function getAccessList() {
        ...
        if (isset($_GET['searchForm']['name']) && trim($_GET['searchForm']['name']) != "") {
            ...
            $whereParam["name"] = StringManage::processString($_GET['searchForm']['name']);
        }
