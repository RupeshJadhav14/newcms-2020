...
<?php echo StringManage::createJsObject("faqData", $data); ?>
...
applyPopover();
        var rules = {
            ...
            // display message
<?php Core::displayMessage("actionResult", "Faq Save"); ?>
...
//    });
    ...
    function fillForm()
    {
        ...
        if (faqData)    // fill form data if passed
        {
            
            
            $("#txtQuestion").val(faqData.question);
            

            // check tinyMCE is initialized or not
            if (tinyMCE.activeEditor == null)
                $("#txaDescription").val(faqData.answer);
            else
                tinyMCE.get("txaDescription").setContent(faqData.answer);

            $("#txtSortOrder").val(faqData.sort_order);
