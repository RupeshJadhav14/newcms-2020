    ...
    <ul class="navUl">
        ...
        foreach ($data as $key => $val) { //echo "<pre>";print_r($val);exit;
          ...
          /*if ((int) $cntMenu[$val['module_key']] > 1) {
            ...
            echo '<li '.$class.'><a href="' . $link . '" title="' . $val['moduleName'] . '" '.$class.'> <span class="iconSpan"><img src="' . URI::getLiveTemplatePath() . '/images/' . $val['icon_class'] . '.png" class="fiImg"><img src="' . URI::getLiveTemplatePath() . '/images/' . $val['icon_class'] . '-active.png" class="secImg"></span>' . $val['moduleName'] . '</a><ul start class="sub "  >';
          //}
        }
        if ($val['moduleName'] != $val['admin_menu_label']) {
          echo '<li><a href="index.php?m=' . $val['module_key'] . '&a=' . $val['action'] . '" title="' . $val['admin_menu_label'] . '"' . $class . '><span class="iconSpan"><img src="' . URI::getLiveTemplatePath() . '/images/' . $val['icon_class'] . '.png" class="fiImg"><img src="' . URI::getLiveTemplatePath() . '/images/' . $val['icon_class'] . '-active.png" class="secImg"></span>' . $val['admin_menu_label'] . '</a></li>';
        }
        if ($key == (count($data) - 1)) {
          echo '</ul></li>';
        }
